	...
	<div id="main">   
		...
		<div id="content">
			...
			<?php
			 ...
			 // 	$classes = "post p200x200 home facebook _all";
			 // 	$id = 81;
			 // 	$rolloverclass = "over-state";
			 // 	$title = "Facebook Feed";
			 // 	$message = "";
			...
			<a data-image="<?php echo $imagepath; ?>" data-id="<?php echo strval($id); ?>" data-title="<?php echo $title; ?>" data-client="" data-detail-id="<?php echo $detail_name; ?>" class="<?php echo $classes;?>">
				<div id="rollover" class="<?php echo $rolloverclass; ?>">
			...
			<!--twitter-->
			<?php
				$t_post 		= $twitter_feed[0];
			 	$imagepath 		= $this->config->item("media_url")."images/200x200/twitter_icon.jpg";
			 	$classes 		= "post p200x200 home twitter _all";
			 	$id 			= 82;
			 	$rolloverclass 	= "over-state";
			 	$title 			= "Twitter Feed";
			 	$message 		= "";

			 	if(!empty($t_post["text"]))
			 		$message = $t_post["text"];
	    ...
	    <div id="post-detail" class="post detail">
			...
			<div id="twitter-detail-content">
				<div id="post_detail_header">
                	<a class="close-btn"></a>
                	<a class="cta" href="<?php echo TWITTER_PAGE_URL; ?>" traget="_blank">Follow us on Twitter</a>
                    <h1>&nbsp;</h1>
                    <h2>&nbsp;</h2>
	            </div>
				<?php 
					...
					foreach ($twitter_feed as $key=>$t_post){
						array_push( $columns[ ($key%2 == 0) ? "even" : "odd" ], $t_post );
					}
				?>
				<?php foreach ($columns as $key=>$column): ?>
				<div class="fb-detail-column">
					<?php foreach($column as $key=>$post){ $this->load->view( "twitter_post_view", array("post"=>$post) ); } ?>
				</div>
				<?php endforeach ?>
			</div>
			<div id="facebook-detail-content">
				
				<div id="post_detail_header">
                	<a class="close-btn"></a>
                	<a class="cta" href="<?php echo str_replace("graph", "www", FB_PAGE_URL); ?>" traget="_blank">Visit us on Facebook</a>
                    <h1>&nbsp;</h1>
                    <h2>&nbsp;</h2>
	            </div>
				...
				<?php 
					...
					foreach ($fb_feed as $key=>$fb_post){
						array_push( $columns[ ($key%2 == 0) ? "even" : "odd" ], $fb_post );
					}
				?>

				<?php foreach ($columns as $key=>$column): ?>
				<div class="fb-detail-column">
					<?php foreach($column as $key=>$post){ $this->load->view( "facebook_post_view", array("post"=>$post) ); } ?>
				</div>
				<?php endforeach ?>
			</div>
		</div>

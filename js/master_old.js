...
main.local 						= main.domain == "click3x.dev" ? true : false;
...
main.routes.postVideosDir		= "http://click3x.com/media/video/";
main.routes.postImagesDir 		= "http://click3x.com/media/images/";
...
main.facebook.settings			= 	{ appid: ( main.local ? "131020827093243" : "201511586639916" ) };
...
main.pages.headings 	= 	{	"all":"SEARCH RESULTS",
								"home":"HOME",
								"portfolio":"PORTFOLIO",
								"who-we-are":"WHO WE ARE",
								"news":"CURRENT EVENTS",
								"clients":"Fashion & Beauty Brands We've Worked With",
								"contact":"GIVE US A CALL, OR STOP BY"
							};
...
jQuery( function($){
	...
	//Init Isotope.
    $("#content").isotope(main.isotope.config);
    
    //Init Facebook.
    FB.init({ appId: main.facebook.settings.appid, status: true, cookie: true });
	...
	//update facebook rss feed
	//main.facebook.updateFeed();
...
main.facebook.publish = function( $e ){
	...
	main.facebook.postdata.picture = "http://69.25.178.3/staging/master/" + $("#post-detail #related_assets li.selected img").attr("src");
	main.facebook.postdata.description = $(".post.selected .over-state p").text();
	main.facebook.postdata.link = "http://69.25.178.3/staging/master/#" + main.address.pathNames.join("/");
...
main.facebook.updateFeed = function(){
	//var profile_id = "1234567890";		
 
	//App Info, needed for Auth
	var app_secret = "5742c07f214cc46ec28e62d58f554a0d";
	 
	var authToken;

	// fetch profile feed with the Auth token appended
	//var data = fetchUrl("https://graph.facebook.com/{$profile_id}/feed?{$authToken}");

	$.ajax({
		url: "https://graph.facebook.com/oauth/access_token?type=client_cred&client_id={" + main.facebook.settings.appid + "}&client_secret={" + app_secret + "}",
		type: "GET",
		dataType: "json",
...
main.twitter.publish = function( $e ){
	...
	console.log("-- Bitly Complete -- : " + main.bitly.current);
	main.twitter.postdata.status = escape(	"Check out " + 
											$("#post_detail_header h2").text()) + " " + 
											$("#post_detail_header h1").text() + ", by Raison D'Être : " + 
											main.bitly.current;
	
	window.open( 	"https://twitter.com/intent/tweet?source=webclient&text=" +
					main.twitter.postdata.status, "_blank", "width=500,height=300,top=200px,left=" +
					$(window).width()*.5);
	
	return false;
}

main.bitly.shorten = function( $complete ){
	$.ajax({
		url: main.bitly.url + "http://69.25.178.3/staging/master/#" + main.address.pathNames.join("/"),
		type: "GET",
		dataType: "json",
		success: function( $response ) {
			$complete( $response.data.url );
		},
		error: function( $error ){
			console.log( $error );
		}
...
main.address.change	= function (){
	...
	if( main.curPathLevels >= 1){
		...
		if( main.filters.page != "home" ){
			main.filters.page = "home";
			pageChanged = true;
		}
	}
	...
	if( main.curPathLevels >= 2 ){
		...
		if(num_categories != 0){
			default_category = $(".page-header[data-page='" + main.filters.page + "'] li:first-child a").attr("data-id");
		} 
	...
	/* ----------------------------------------------------------------------------------------------------------------
	Hack: This is a hack for now to display the proper category menu on the Portfolio page. 
	TODO: Make dynamic.
	---------------------------------------------------------------------------------------------------------------- */
	//$("#page-header a.dropdown-toggle").text( main.filters.category );	
	//$("#page-header h1").text( main.filters.page.toUpperCase().replace(/-/g,' ') );
    
    //if( main.filters.page == "portfolio" || main.filters.page == "all"){
    //	if($("#filter-menu").hasClass("hidden"))
	...
	//$("#page-header h1").text( main.pages.headings[main.filters.page].toUpperCase() );
	
	//get current bitly url.
	//if(main.curPathLevels >= 3){
	//	main.bitly.shorten( function( $url ){
	//		console.log("bitly updated : " + $url);
	//		main.bitly.current = $url;
	//	});
	//}
	...
	if(!main.local){
		if(main.curPathLevels > 0){
			ga('send', 'pageview', main.address.pathNames.join("/"));
		}
	}
	...
	//$(".page-header .subcategory-menu").css("display","none");
}
...
main.search.suggestionClicked = function(){
	var li = $(this);
	var text = li.children(":first").text();

	$("#header_search_input").val(text.toUpperCase());
...
main.search.change		= function(){
	/*clearTimeout(main.timers.search);
	main.timers.search = setTimeout( main.search.search, 400);*/
}

main.search.search		= function(){
	/*var val = 	$("#header_search_input").val().toLowerCase();
	console.log("search for this shit :: " + val);
	
	$.address.value( "search" ).parameter("tag", val, false)*/
}

main.search.update		= function(){
	/*var val = 	$("#header_search_input").val();
	var menu = 	$("#header_search_dropdown");
	
	menu.children("li .suggestion").unbind("click").remove();
	
	$.each( main.search.current, function(i)
	{
		if( val.length > 0 && this.indexOf( val ) != -1 ){
		    var li = $( '<li/>' ).addClass("suggestion").prependTo( menu ).click(main.search.suggestionClicked);
		    var aaa = $( '<a/>' )
		        .attr( 'role', 'menuitem' )
		        .attr( 'tabindex', "-1" )
		        .addClass( 'ui-all' )
		        .text( main.search.current[i].toUpperCase() )
		        .appendTo( li );
		 }
	});*/
};
...
main.post.load	= function(){
	/* ---------------------------------------------------------------------------------------------------------------- 
	Hack: Checck to see if we've already loaded the posts. 
	TODO: Detect if all of the filtered posts have been loaded.
	---------------------------------------------------------------------------------------------------------------- */
	if($("#content .post").length == 0){
		$.ajax({
			url: data_directory + "/posts.json",
			type: "GET",
			dataType: "json",
			success: function( $data ) {
				main.currPostCount = $data.posts.length;
				
				console.log("posts complete - we were given " + main.currPostCount + " posts");	
				$($data.posts).each( main.post.build );
			},
			error: function( $error ){
				console.log( $error );
			}
		...
		main.post.loadComplete();
	}
};

main.post.build	= function($i, $p){
	/* ----------------------------------------------------------------------------------------------------------------
	Force iPhone to be 200x200
	---------------------------------------------------------------------------------------------------------------- */
	var _size = main.settings.iphone == true ? "200x200" : $p.size;
	...
	var post = $("<a />")
		.attr({	"data-id"			:$p.id,
				...
				"data-detail-id"	:$p.detail_name
		})
		.addClass( "p" + _size )
		.addClass( "post" )
		.addClass( $p.pages.join(" ") )
		.addClass( $p.categories.join(" ") )
		.addClass( $p.tags.join(" ") )
	
	//check for href else add the click event
	if($p.href != undefined && $p.href != ""){
		post.attr({"href":$p.href, "target":"_blank"});
	} else {
		if($p.detail_name != undefined && $p.detail_name != ""){
			post.click( main.post.click );
		}
	}
	...
	if($p.filename != undefined && $p.filename != ""){
		...
		//check for gif
		if($p.media_type != undefined && $p.media_type == "3")
			type = "gif";
			
		$( img ).attr( "src", main.routes.postImagesDir + _size + "/" + $p.filename + "." + type );
		post.append(img);
	} else{
		post.addClass("no-image");
	}
	
	/* ----------------------------------------------------------------------------------------------------------------
	Build Rollover HTML
	---------------------------------------------------------------------------------------------------------------- */
	var rollover = $("<div />").addClass("over-state");

	if( $p.title != undefined && $p.title != "" )
		$("<h1 />").html($p.title).appendTo(rollover);

	if( $p.client != undefined && $p.client != "" )
		$("<h2 />").html($p.client).appendTo(rollover);

	if( $p.description != undefined && $p.description != "" )
		$("<p />").html($p.description).appendTo(rollover);


	rollover.prependTo(post);
	
	/* ----------------------------------------------------------------------------------------------------------------
	Add Rollover State Classes 	"fixed"	: 	Always show the rollover div.
								"half" 	: 	Rolover only covers half of the thumbnail. Doesn't apply to 200x200.
								"none" 	: 	Never show the rollover state.
	---------------------------------------------------------------------------------------------------------------- */
	if($p.overstatestyle != undefined)
		rollover.addClass($p.overstatestyle);

	//remove fixed if iPHone since it doesn't apply to 200x200
	if(main.settings.iphone == true && rollover.hasClass("fixed"))
		rollover.removeClass("fixed");
	
	//Insert the post into isotope.
	$("#content").isotope( 'insert', post, function($e){
		console.log("-- Insert Post Complete --");
	});
	
	//call loadPostsComplete on the last post.
	if(main.currPostCount-1 == $i){
		main.post.loadComplete();
	}

	//add hover image
	if($p.hovericon != undefined && $p.hovericon!="" ){
		var size =  (rollover.hasClass("fullimg")) ? $p.size : "200x200";
		if(rollover.hasClass("half")){
			size = ($p.size == "410x410") ? "410x200" : "200x200";
		}
		$("<img />").attr( {"src": main.routes.postImagesDir + "/"+size+"/" + $p.hovericon + ".jpg", width:80, height:80} ).prependTo(rollover);
	}
}; 

main.post.loadComplete = function (){
	console.log("-- Load Posts Complete --");
	...
	$("#content").isotope( 'insert', element, function(){
		...
		setTimeout(function(){ 
			main.detail.load( function(){
				...
				main.content.reLayout(function(){
					main.detail.transitionIn(function(){
...
main.detail.transitionIn = function( $complete ){
	var element = $( "#post-detail" );
...
main.detail.reset = function(){
	...
	$("#content").isotope( 'insert', element, function(){
		console.log("remove detail complete");
	});
}
...
main.detail.buildRelatedPosts	= function($i, $a){
	var item 	= $("<li />").attr({ "data-filename": $a.filename , "data-id":$a.id, "data-type":$a.media_type });
	var a		= $("<a />").attr({ "href": "#" + $a.page + "/all/" + $a.id}).appendTo(item);
...
main.detail.asset.deselectAll = function(){
	...
	$("#video_player img").attr("src", "../img/blank_img.jpg");
	$("#post_detail_main #detail_image").attr("src", "../img/blank_img.jpg");
}
